#!/usr/bin/env python3
"""Focused robustness search for DYDX trend-pullback short.

The first new-family scan found DYDX trend_pullback_short as the only promising
new idea, but stress execution turned 730d slightly negative. This script tests
a compact robustness grid around the same idea without changing the family:

- short only;
- 1h+4h downtrend background;
- pullback into EMA20/EMA50;
- RSI band;
- optional return guards;
- TP/SL/time variants;
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tune DYDX trend-pullback short robustness.")
    parser.add_argument("--raw-1m-dir", default="data/futures_1m_raw_history")
    parser.add_argument("--save-summary", default="data/dydx_pullback_short_tune_summary.csv")
    parser.add_argument("--save-windows", default="data/dydx_pullback_short_tune_windows.csv")
    parser.add_argument("--save-report", default="strategies/dydx-pullback-short-tune.md")
...
def variants():
    out = []
    for rsi_min, rsi_max in [(42, 62), (45, 60), (48, 62), (50, 65)]:
        for ema20_low, ema50_high in [(0.997, 1.008), (0.999, 1.006), (1.000, 1.010)]:
            for atr_min, atr_max in [(0.0015, 0.018), (0.0020, 0.014), (0.0025, 0.012)]:
                for vol_mult in [0.0, 1.0, 1.2]:
                    for guard in ["none", "avoid_after_dump", "only_after_pump"]:
                        for tp, sl, time_stop in [(0.004, 0.018, 90), (0.005, 0.020, 120), (0.006, 0.025, 180), (0.007, 0.030, 180)]:
                            name = (
                                f"rsi{rsi_min}-{rsi_max}_ema{ema20_low}-{ema50_high}_atr{atr_min}-{atr_max}"
                                f"_vol{vol_mult}_guard{guard}_tp{tp}_sl{sl}_t{time_stop}"
                            )
                            out.append(
                                {
                                    "variant": name,
...
def signal_for(df, v):
    sig = (
        ...
        & df["htf240_short"]
        & (df["close"] < df["ema200"])
        & (df["close"] >= df["ema20"] * v["ema20_low"])
        & (df["close"] <= df["ema50"] * v["ema50_high"])
        & df["rsi14"].between(v["rsi_min"], v["rsi_max"])
        & df["red"]
        & df["atr_pct"].between(v["atr_min"], v["atr_max"])
        & (df["upper_wick_ratio"] < 0.45)
    )
    if v["vol_mult"] > 0:
        sig &= df["volume"] > df["vol_sma20"] * v["vol_mult"]
    if v["guard"] == "avoid_after_dump":
        sig &= (df["ret_24h"].fillna(0) >= -0.06) & (df["ret_7d"].fillna(0) >= -0.15)
    elif v["guard"] == "only_after_pump":
        sig &= (df["ret_24h"].fillna(0) >= 0.00) & (df["ret_7d"].fillna(0) >= -0.05)
    return sig.to_numpy()
...
def summarize(trades, curve):
    ...
    return {
        "trades": len(trades),
        "return_pct": (curve[-1] / INITIAL_BALANCE - 1) * 100,
...
def run_backtest(df, signal, v, position_pct, fee_pct, slippage_pct):
...
def build_summary(windows):
    grouped = {}
    for row in windows:
        key = (row["variant"], row["scenario"], row["position_pct"])
        grouped.setdefault(key, []).append(row)
    out = []
    for key, rows in grouped.items():
        by = {r["period"]: r for r in rows}
        ref = by.get("730d") or by.get("365d")
        if not ref:
            continue
...
def save_report(path, summary, windows):
    ...
    strict = [
        ...
        and float(r.get("pf_730d") or 0) >= 1.0
    ...
    for i, r in enumerate(strict[:80], 1):
        lines.append(
            f"| {i} | {float(r['position_pct']):.0%} | `{r['variant']}` | {r['positive_windows']}/{r['windows_count']} | "
            f"{fmt(r['return_365d_pct'])}% | {fmt(r['return_730d_pct'])}% | "
...
def main():
    ...
    positions = [0.10, 0.15, 0.25]
    vars_ = variants()
    windows_out = []
    for idx, v in enumerate(vars_, 1):
        signal = signal_for(df, v)
        if signal.sum() < 200:
            continue
        if idx % 50 == 0:
        ...
        for scenario, fee_pct, slippage_pct in scenarios:
            for position_pct in positions:
                for days in WINDOWS:
                    ...
                    summary = run_backtest(sub, sub_signal, v, position_pct, fee_pct, slippage_pct)
                    windows_out.append(
                        {
                            **v,
                            "symbol": "DYDXUSDT",
